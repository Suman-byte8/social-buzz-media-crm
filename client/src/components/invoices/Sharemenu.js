  ...
  return (
    <div className="relative ml-auto" ref={menuRef}>
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        disabled={!hasClient}
        title={hasClient ? "Share invoice" : "Select a client to share"}
        className="rounded border border-white/30 px-3 py-1.5 text-[12px] font-semibold tracking-wide hover:bg-white hover:text-ink focus:outline-none focus:ring-2 focus:ring-white/60 disabled:cursor-not-allowed disabled:opacity-50"
      >
        Share
      </button>

      {open && (
        <div className="absolute right-0 top-[calc(100%+6px)] z-40 w-64 rounded-md border border-black/10 bg-white p-2 text-ink shadow-lg">
          <div
          ...
          {isSending && (
            <p className="mt-1 px-2 text-[11px] text-[#6E6A65]">
